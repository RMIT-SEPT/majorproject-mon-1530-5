  ...
  render() {
    ...
    const previousMonthLastDays = () => {
      ...
      for (let i = firstDayIndex; i > 0; i--) {
        previousDays.push(
          ...
          <td className="previousDays" key={i}>
            {prevLastDay - i + 1}
          </td>
        );
      }
    ...
    const nextMonthDays = () => {
      ...
      for (let i = 1; i <= nextDays; i++) {
        nextMonthDays.push(<td className="nextMonth" key={i}>{i}</td>);
      }
    ...
    const createFirstRow = () => {
      ...
      for (let i = 1; i <= 7 - previousMonthLastDays().length; i++) {
        if(selectedDay === i){
          rows.push(<td className="currentMonth" onClick={this.setDay} key={i}>{i}</td>);
        }
        else if(availableDays.includes(i)){
          rows.push(<td className="currentMonth" onClick={this.setDay} key={i}>{i}</td>);
        }
        else{
          rows.push(<td className="currentMonth" key={i}>{i}</td>);
        }
   ...
   // Creats a middle rows of the calendar 
    const createMiddleRows = (n) => {
      ...
      for (let i = 1; i <= 7; i++) {
        ...
        if (j <= lastDay) {
          if(selectedDay === j){
            rows.push(
              <td className="availableDays" key={j}>
                {j}
              </td>
            );
          }
          else if(availableDays.includes(j)){
            rows.push(<td className="availableDays" onClick={this.setDay} key={j}>{j}</td>);
          }
          else{
            rows.push(
              <td className="unavailableDays" key={j}>
                {j}
              </td>
            );
          }
    ...
    const createLastRow = () => {
      ...
      if (lastDayIndex !== 6) {
        if (previousMonthLastDays().length === 5) {
          if(selectedDay === 31){
            rows.push(
              <td className="currentMonth" key={31}>
                {31}
              </td>,
              nextMonthDays()
            );
            ...
            rows.push(
              <td className="availableDays" onClick={this.setDay} key={31}>{31}</td>,
              nextMonthDays()
              );
          } else {
            rows.push(
              <td className="unavailableDays" key={31}>
                {31}
              </td>,
              nextMonthDays()
            );
          ...
          if(selectedDay === 30){
            rows.push(
              <td className="previousDays" key={30}>
                {30}
              </td>
            );
          } else if (availableDays.includes(30)) {
            rows.push(
              <td className="availableDays" onClick={this.setDay} key={30}>{30}</td>
              );
          } else {
            rows.push(
              <td className="unavailableDays" key={30}>
                {30}
              </td>
            );
          }
          if(selectedDay === 31){
            rows.push(
              <td className="previousDays" key={31}>
                {31}
              </td>,
              nextMonthDays()
            );
    ...
    return (
      <div className="max-w-lg container">
        <div className="flex justify-evenly bg-blue-900">
          <svg
